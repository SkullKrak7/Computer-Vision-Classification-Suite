...
def train_tensorflow(data_dir='datasets/custom', epochs=20, fine_tune=False):
    ...
    model.train(X_train, y_train, labels, X_val=X_test, y_val=y_test,
...
if __name__ == "__main__":
    ...
    parser.add_argument('--model', type=str, choices=['pytorch', 'tensorflow', 'both'],
                       default='pytorch', help='Model to train')
    parser.add_argument('--data', type=str, default='datasets/custom',
                       help='Path to dataset directory')
    parser.add_argument('--epochs', type=int, default=20,
                       help='Number of training epochs')
    parser.add_argument('--fine-tune', action='store_true',
                       help='Fine-tune MobileNet layers (TensorFlow only)')
