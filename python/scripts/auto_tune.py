...
class ModelTuner:
    ...
    def tune_iteration(self, config, X_train, y_train, X_val, y_val, label_map, iteration):
        ...
        model.train(X_train_aug, y_train_aug, label_map, 
        ...
        logger.info(f"Train Acc: {metrics['train_acc']:.4f}, Val Acc: {metrics['val_acc']:.4f}, Gap: {metrics['gap']:.4f}")
        
        # Determine if optimal
        is_optimal = (
            metrics['gap'] < 0.15 and  # Not overfitting
            metrics['val_acc'] > 0.65 and  # Good performance
            metrics['val_f1'] > 0.60  # Good F1 score
        )
    ...
    def tune(self, max_iterations=5):
        ...
        if 'PyTorch' in self.model_name:
            ...
            config = {
                'model_params': {'learning_rate': 0.0001, 'use_mixed_precision': True, 'fine_tune': False},
                'epochs': 5,
                'batch_size': 16,
                'augment': False
            }
